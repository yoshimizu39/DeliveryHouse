using DeliveryHouse.Common.Request;
    ...
    public class AccountController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateToken([FromBody] LoginViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    Microsoft.AspNetCore.Identity.SignInResult signInResult = await _userHelper.ValidatePasswordAsync(user, model.Password);
        ...
        public async Task<IActionResult> GetUserByEmail([FromBody] EmailRequest emailRequest)
        {
            ...
            User user = await _userHelper.GetUserAsync(emailRequest.Email);

            if (user == null)
            {
                return NotFound("Error 001");
            }
